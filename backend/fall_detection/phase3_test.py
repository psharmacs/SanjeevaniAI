...
# ================================================================
# SANJEEVANI NEED HELP — VOICE COMMAND
# ================================================================
# Direct voice emergency is independent of the existing Phase 1,
# Phase 2, and Phase 3 state machines.
VOICE_WAKE_WORD = "sanjeevani"
VOICE_COOLDOWN_SECONDS = 10.0
NEED_HELP_OVERLAY_SECONDS = 5.0
...
# Explicit sounddevice input device index to record from, instead
# of relying on Windows' current default input device (which can
# be wrong — e.g. set to a virtual/webcam-app mic that captures
# silence, as happened during testing). None = fall back to
# whatever sounddevice picks as default.
#
# Find your correct device index by running find_mic.py, which
# tests each candidate microphone and reports which one actually
# picks up sound.

MICROPHONE_DEVICE_INDEX = 7

# pyttsx3's default speaking rate (~200 words/min) is quite fast
# for a check-in prompt someone may be startled or groggy hearing.
# Slower is clearer. ~150 is a commonly recommended comfortable
# rate; drop lower (e.g. 130) if it's still too fast.
...
def start_voice_assistant():
    """Start the independent Sanjeevani Need Help listener."""
    if VoiceAssistant is None:
        print("[SANJEEVANI] Need Help voice command disabled — voice_assistant.py not found.")
        return None

    try:
        assistant = VoiceAssistant(
            wake_word=VOICE_WAKE_WORD,
            cooldown_sec=VOICE_COOLDOWN_SECONDS,
        )
        assistant.start()
        if assistant.is_available():
            print("[SANJEEVANI] Need Help voice command: ENABLED")
            return assistant
        print("[SANJEEVANI] Need Help voice command: DISABLED (microphone unavailable)")
    except Exception as error:
        print(f"[SANJEEVANI] Need Help voice command failed to start: {error}")
    return None


def check_voice_need_help():
    """Poll for a direct voice emergency without changing fall-detection state."""
    global need_help_until, need_help_command

    if voice_assistant is None:
        return False

    try:
        detected, command = voice_assistant.consume_emergency()
    except Exception as error:
        print(f"[SANJEEVANI] Need Help polling error: {error}")
        return False

    if not detected:
        return False

    need_help_command = command
    need_help_until = time.time() + NEED_HELP_OVERLAY_SECONDS

    print()
    print(f'>>> [SANJEEVANI] NEED HELP DETECTED — command: "{command}" <<<')
    print(">>> [SANJEEVANI] Voice emergency registered; Phase 1/2/3 state unchanged. <<<")
    return True
...
# Independent voice listener. Failure here never stops the camera pipeline.
...
while True:
    ...
    # ------------------------------------------------------------
    # SANJEEVANI NEED HELP — independent voice event.
    check_voice_need_help()
    ...
    # ------------------------------------------------------------
    # Direct voice emergency overlay.
# Display text requested: 🚨 HELP REQUEST RECEIVED — ALERTING CARETAKER
    #
    # IMPORTANT:
    # This is display-only. It does NOT modify Phase 1/2/3,
    # DANGER, RECOVERY, or escalation state.
    if time.time() < need_help_until:
        cv2.putText(
            display,
            "HELP REQUEST RECEIVED - ALERTING CARETAKER",
            (30, 105),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.85,
            (0, 0, 255),
...
if voice_assistant is not None:
    try:
        ...
        print(f"[SANJEEVANI] Voice assistant shutdown warning: {error}")
