...
DANGER_MOVEMENT_PX = 8     # px/frame threshold for "still" in danger check

# ── RESPONSIVE / RECOVERY-TRIGGER threshold ──────────────────────────
# Once DANGER has fired, this is the movement level that counts as
# "the person moved their leg/limb" and should trigger RECOVERY, even
# though the torso itself is still flat on the ground.
LEG_MOVE_PX        = 10    # px/frame — counts as deliberate movement
LEG_MOVE_FRAMES    = 2     # consecutive-ish frames needed (debounce noise)

# ── RECOVERY display duration ────────────────────────────────────────
# RECOVERY is shown for a fixed window after being triggered, then the
# engine resets and goes back to normal posture-based monitoring —
# regardless of the person's exact pose at that moment.
RECOVERY_DISPLAY_SEC = 10.0
...
class RuleEngine:
    def __init__(self, cooldown_sec=COOLDOWN_SEC):
        ...
        self._danger_triggered = {}   # id → True once DANGER has fired for this fall episode

        # RECOVERY timer
        self._recovery_start   = {}   # id → timestamp RECOVERY state was entered
    ...
    def process(self, object_id, buffer, feature_extractor=None):
        ...
        if danger:
            self._danger_triggered[object_id] = True
        ...
        if state == FallState.NORMAL:
            ...
            if self._check_recovery(object_id, buffer, now):
                # Genuine stand-up/sit-up.
                next_state = FallState.RECOVERY
                self._recovery_start[object_id] = now
            elif (self._danger_triggered.get(object_id, False)
                    and self._check_leg_movement(object_id, buffer)):
                # DANGER already fired for this episode, and the person
                # has now moved a leg/limb — treat as recovery even
                # though the torso is still on the ground.
                next_state = FallState.RECOVERY
                self._recovery_start[object_id] = now

        elif state == FallState.RECOVERY:
            # Fixed 10-second display window. After it elapses, reset
            # everything and resume normal posture-based monitoring —
            # regardless of the person's exact pose at that moment.
            start = self._recovery_start.get(object_id, now)
            if now - start >= RECOVERY_DISPLAY_SEC:
                next_state = FallState.NORMAL
                ...
                self._danger_triggered.pop(object_id, None)
                self._recovery_start.pop(object_id, None)
                self._lying_since.pop(object_id, None)
                self._danger_fired.pop(object_id, None)
        ...
        # No longer restricted to NORMAL/PRE_FALL — a person can be
        # "lying (moving)" while FALL_CONFIRMED too, and the label
        # should reflect that instead of freezing on "FALL DETECTED".
        is_lying_moving = (
            currently_lying
    ...
    # ── Leg/limb movement check (recovery trigger after DANGER) ───────
    def _check_leg_movement(self, object_id, buffer):
        """
        True if the person has just moved (a leg/limb) — checked as the
        last few buffer samples crossing LEG_MOVE_PX, so a single noisy
        frame doesn't false-trigger, but it's still near-immediate.
        """
        if len(buffer) < LEG_MOVE_FRAMES:
            return False
        recent = buffer[-LEG_MOVE_FRAMES:]
        return all(f.get("movement", 0) > LEG_MOVE_PX for _, f in recent)
